...
public class CityMap {
   public String getLegend(String[] cityMap, int[] POIs) {
        char[][] cityMapC=new char[cityMap.length][];
        for (int i=0;i<cityMap.length ;i++ ) {
            cityMapC[i] = cityMap[i].toCharArray();
        }
        HashMap<Character, Integer> dic = new HashMap<Character, Integer>();
        for(int i=0;i<cityMap.length ;i++ ){
            for (int j=0;j<cityMapC[i].length ;j++ ) {
                dic.put(cityMapC[i][j],0);
            }
        }
        for(int i=0;i<cityMap.length ;i++ ){
            for (int j=0;j<cityMapC[i].length ;j++ ) {
                dic.put(cityMapC[i][j],dic.get(cityMapC[i][j])+1);
            }
        }
        char[] ans= new char[POIs.length];
        for(int i=0;i<POIs.length ;i++ ){
            for (Character key :dic.keySet() ) {
                if(POIs[i]==dic.get(key)){
                    if(dic.get(key)=='.')continue;
                    ans[i]=key;
                    break;}
             }
        }

...
public class TeamContestEasy {
    public int worstRank(int[] strength) {
        ...
        for (int i=0;i<strength.length;i++)
            S.add(strength[i]);
        Collections.sort(S);
        System.err.print("@@"+S.get(1));

        int teammin = Math.min(Math.min(strength[0], strength[1]), strength[2]);
        int mine = strength[0] +strength[1] + strength[2] - teammin;
    ...
    public int teamVal(ArrayList<Integer> s, int i, int j, int k) {
        int teamval = 0;
        int teammin = Math.min(Math.min(s.get(i), s.get(j)), s.get(k));
        teamval = s.get(i) +s.get(j) + s.get(k) - teammin;
        return teamval;
    }

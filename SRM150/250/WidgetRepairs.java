...
public class WidgetRepairs {
   public int days(int[] arrivals, int numPerDay) {
       ...
       int ans = 0;
       System.err.print("[ " + idle + " ]");
       System.err.print("[ " + (int)ceil(W/numPerDay) + " ]" + W +"--");
       ans = arrivals.length + (int)round((double)W/(double)numPerDay) - idle;
       return ans;
...
class WidgetRepairsHarness {
	...
	static int runTestCase(int casenum__) {
		...
		case 0: {
			...
			return verifyCase(casenum__, expected__, new WidgetRepairs().days(arrivals, numPerDay));
		}
		case 1: {
			int[] arrivals            = { 0, 0, 0 };
			int numPerDay             = 10;

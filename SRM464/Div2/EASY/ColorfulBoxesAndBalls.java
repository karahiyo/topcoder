public class ColorfulBoxesAndBalls {
    public int getMaximum(int numRed, int numBlue, int onlyRed, int onlyBlue, int bothColors) {
        int ret = 0;
        ret = numRed * onlyRed + numBlue * onlyBlue;
        int dif = numRed - numBlue;
        if(dif >= 0) {
            ret = Math.max(ret, dif * onlyRed + (numRed + numBlue - dif) * bothColors);
        } else {
            ret = Math.max(ret, -dif * onlyBlue + (numRed + numBlue + dif) * bothColors);
        }
        return ret;
